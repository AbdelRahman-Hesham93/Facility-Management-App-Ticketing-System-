...
class EventService extends StatefulWidget {
 ...
 Future<List<String>> fetchCollection(String collectionName) async {
  ...
  @override
  EventServiceState createState() => EventServiceState();
}

class EventServiceState extends State<EventService> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(  backgroundColor: const Color.fromARGB(255, 201, 216, 197),
      ...
      appBar:
          AppBar(title: const Text("Event Request"), centerTitle: true,
           leading: Image.asset(
      ...
      child: Column(
        ...
        children: [
          ...
          const CircleAvatar(
            radius: 50,
            backgroundImage: NetworkImage('https://iili.io/JHvsFp4.png',
),
          ),
          DropdownButtons(
                collectionName: 'ERequest',
                hintText: 'Choose your Event',
                onChanged: (value) {
          ...
          Row(
            ...
            MyTextBox(
              ltxt: 'Requirments',
              rtxt: 'Please enter your requirments',
              controller: reqCont,),
              const Padding(padding: EdgeInsets.symmetric(vertical: 5)),
             MyTextBox(
...
final formattedDateTime = DateFormat('yyyy-MM-dd HH:mm').format(submitDate);
                CollectionReference collRef =
                    FirebaseFirestore.instance.collection('EventRequests');
                collRef.add({
                  'Requirments': reqCont.text,
